...
- **Mark uncertainty.** If you inferred a *positive* claim without reading the file, label it `[inferred]`. The next agent must be able to tell verified facts from educated guesses; if you blur that line you've poisoned its context.
- **"Absence" claims need a different verification mode than "presence" claims.** Saying "no CI configured" is verified by `ls .github/workflows/` returning nothing — note the verification command rather than tagging `[inferred]`. Reserve `[inferred]` for *positive* claims you couldn't directly verify.
